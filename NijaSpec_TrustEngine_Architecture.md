...
// To regenerate, re-run nija-audit generate
...
`;
    ...
    if (route.requiresAuth) {
      testCode += `      req.set('Authorization', 'Bearer mock-token-nija-audit');
`;
    }
...
- [ ] Wrap the compiler in a CLI interface: `nija-audit generate <spec.md> <output.spec.js>`
- [ ] Add error messages for invalid schemas
- [ ] Add a `--dry-run` flag to preview output without writing
- [ ] Publish v0.1.0 to npm

**Output:** Users can `npm install -g nija-audit` and use the tool
