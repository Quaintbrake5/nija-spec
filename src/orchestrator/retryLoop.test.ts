...
describe('RetryLoop', () => {
  it('should succeed on first attempt', async () => {
    ...
    const result = await RetryLoop.execute(task);
  ...
  it('should throw after max retries', async () => {
    ...
    await expect(RetryLoop.execute(task, 3)).rejects.toThrow('Task failed after 3 attempts');
    expect(task).toHaveBeenCalledTimes(3);
  });

  it('should default to 3 retries', async () => {
    const task = jest.fn().mockRejectedValue(new Error('fail'));
    await expect(RetryLoop.execute(task)).rejects.toThrow();
    expect(task).toHaveBeenCalledTimes(3);
  });
